...
import { listByStatus, isValidId } from "@/lib/firestoreHelpers";
import { getServerSession } from "next-auth";
import { authOptions } from "@/lib/auth";
import { cabinetMemberSchema } from "@/lib/validation/cabinetMember";
...
export async function GET(request) {
  ...
  const items = await listByStatus(db.collection("cabinetMembers"), {
...
export async function POST(request) {
  ...
  const parsed = cabinetMemberSchema.safeParse(body);
  if (!parsed.success) return Response.json({ error: parsed.error.flatten() }, { status: 400 });

  if (parsed.data.parentId && !isValidId(parsed.data.parentId)) {
  ...
  const ref = await db.collection("cabinetMembers").add(doc);
