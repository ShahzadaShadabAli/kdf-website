...
// How many applicants the admins have added as members through the website's
// form, by type. Only counts leave the server — never names or details.
export async function getMemberStats() {
  ...
  const snapshot = await db
    ...
    .get();
  const stats = { honorary: 0, permanent: 0, total: snapshot.size };
  snapshot.forEach((doc) => {
    const type = doc.get("membershipType");
